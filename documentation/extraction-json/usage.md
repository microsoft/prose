...
The **Extraction.Json** primary learning API is `Learn` in the `Session` class.

The **Extraction.Json** program is defined in `Extraction.Json.Program` class.
Its key method is `Run()` to execute the program on some input Json to obtain the extracted output.
