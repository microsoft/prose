...
from transformers import (CONFIG_MAPPING, MODEL_MAPPING, Adafactor,
                          HfArgumentParser, get_scheduler)
from src.eval_utils import getEvalDict, getAvgPerf, dumpid2pred, getOverallPerf, dumpOverallresDict
...
class TrainingArguments:
    ...
    report_to: str = field(default="tensorboard", metadata={"help": "The service to report results to."})
    max_train_steps: Optional[int] = field(
        default=None, metadata={"help": "If set, overrides num_train_epochs to compute the number of training steps."}
    )
    ...
    resume_from_checkpoint: Optional[str] = field(
        default=None,
        metadata={"help": "The path to a directory containing model and tokenizer files to resume training from"}
    )
    fp16: bool = field(
        default=False, metadata={"help": "Whether to use 16-bit (mixed) precision (through NVIDIA apex) instead of 32"}
    )
...
class DataTrainingArguments:
    ...
    splits_path: Optional[str] = field(default="splits_50.json", metadata={"help": "Train-Dev-Eval Split path"})
    ...
    predict_complete_sequence: bool = field(
        default=False,
        metadata={"help": "Whether or not to predict the complete sequence. If False, only predict the masked tokens."},
    )
    use_all: bool = field(
        default=False,
        metadata={"help": "Choose whether to use all edits in an edit sequence as target edit or only the one originally intended to be"}
    )
    shuffle: bool = field(
        default=False,
        metadata={"help": "Shuffle the order in which associated edits appear randomly"}
    )
    no_ascEdits : bool  = field(
        default= False 
    )
    def __post_init__(self):
...
def batch_generate(batch, model, max_seq_length, topK = 5, beam_search = False, temperature = 0.2 ):
    batch_input_ids = batch["input_ids"]
    if beam_search:
        outputs = model.module.generate(batch_input_ids, num_beams = topK, max_length = max_seq_length, num_return_sequences = topK)
    else:
        outputs = model.module.generate(batch_input_ids, do_sample = True, max_length = max_seq_length, temperature = temperature, num_return_sequences = topK)
    return outputs    
...
def GetCodeEditDatasetDicts(root_dir, repos, dataset_args: DataTrainingArguments):
    return {repo : AssociatedEditsDataset(os.path.join(root_dir, repo + ".json"), use_all=dataset_args.use_all, shuffle= dataset_args.shuffle, use_ascEdits= not dataset_args.no_ascEdits) for repo in repos}
    
def FlattenDatasetDict( datasetDict):
    return ConcatDataset([v for _,v in datasetDict.items()])
    
def TestAndDumpResults(accelerator, model, tokenizer, dataLoaderDict, max_seq_length, dump_path, topK = 5, beam_search = False, temperature = 0.2):
    idx2idmap = {}
    idx2prefixmap = {}
    idx2suffixmap = {}
    idx2fvmap = None
    ...
    for k, dataloader in dataLoaderDict.items():
        ...
        idx2prefixmap[k] = [prefix for batch in dataloader for prefix in batch["prefix"]]
        idx2suffixmap[k] = [suffix for batch in dataloader for suffix in batch["suffix"]]
        idx2fvmapK = [fv for batch in dataloader for fv in batch.get("futureVersions", [])]
    ...
    for k in dataLoaderDict:
        ...
        # print(f"Dist. Dataloader for {k} {accelerator.process_index}")
    ...
    for k, dataloader in dataLoaderDict.items():
        ...
        progress_bar = tqdm(range(len(dataloader)), disable=not accelerator.is_local_main_process, desc = F"Evaluating {k}")
        for step, batch in enumerate(dataloader):
            with torch.no_grad():
                outputs = batch_generate(batch, model, max_seq_length, topK, beam_search, temperature)
                labels = batch["labels"]
                labels[labels == -100] = tokenizer.pad_token_id
                outputs = torch.nn.functional.pad(outputs, (0, max_seq_length - outputs.shape[-1]), value = tokenizer.pad_token_id)
            
            predDict["topPreds"].extend([GetSpan(pred)for pred in tokenizer.batch_decode(accelerator.gather(outputs), skip_special_tokens=False)])
            predDict["targets"].extend([GetSpan(pred) for pred in tokenizer.batch_decode(accelerator.gather(labels), skip_special_tokens=False)])
        ...
        if accelerator.is_main_process:
            ...
            predDict["topPreds"] = [predDict["topPreds"][i*topK:i*topK +topK] for i in range(len(predDict["topPreds"])//topK)]
            predDict["topPreds"] = [[ prefix + pred + suffix for pred in preds] for prefix, suffix, preds in zip(idx2prefixmap[k], idx2suffixmap[k], predDict["topPreds"])]
            predDict["targets"] = [ prefix + tgt + suffix for prefix, suffix, tgt in zip(idx2prefixmap[k], idx2suffixmap[k], predDict["targets"])]
            logger.info(f"{accelerator.process_index}: Decoding Labels")
            dumpFV = False
            if idx2fvmap is not None:
                iwPredDicts = [{"topPreds": [tp], "targets": [tgt], "futureVersions": fv} for tp, tgt, fv in zip(predDict["topPreds"], predDict["targets"], idx2fvmap[k])]
                dumpFV = True
            else: 
                iwPredDicts = [{"topPreds": [tp], "targets": [tgt]} for tp, tgt in zip(predDict["topPreds"], predDict["targets"])]
            iwEvalDicts = [getEvalDict(iwPredDict) for iwPredDict in iwPredDicts]
            resDict = {id : {"eval": evalDict, "pred": predDict} for id, evalDict, predDict in zip(idx2idmap[k], iwEvalDicts,  iwPredDicts)}
    ...
    if accelerator.is_main_process:
        ...
        logger.info("=====================================")
        if dumpFV:
            logger.info("Raw Top-1 Acc || Raw Top-5 Acc || FV Top-1 Acc || FV Top-5 Acc ||")
            logger.info(
...
def dump_model(accelerator, model, tokenizer, dump_dir):
    ...
    unwrapped_model.save_pretrained(
        dump_dir, is_main_process=accelerator.is_main_process, save_function=accelerator.save
    )
    if accelerator.is_main_process:
...
def main():
    ...
    if model_args.ag_tokenizer:
        tokenizer_kwargs["additional_special_tokens"] = tokenizer_kwargs.get("additional_special_tokens", []) + ["<before>", "</before>","<prefix>", "</prefix>", "<suffix>", "</suffix>","<after>", "</after>","<edit>", "</edit>","<ctxEdits>", "</ctxEdits>"]
    ...
    def data_collator(batch):
        ...
        tokenized_batch_dict = { k: tokenizer(batch_dict[k],
                                        padding="max_length" if data_args.pad_to_max_length else "longest",
                                        max_length=data_args.max_seq_length,
                                        truncation=True,
                                        return_tensors="pt")
                                    for k in ["prompt", "labels"]}
        tokenized_batch_dict["labels"].input_ids[tokenized_batch_dict["labels"].input_ids == tokenizer.pad_token_id] = -100
    ...
    # DataLoaders creation:
        train_dataset = FlattenDatasetDict(GetCodeEditDatasetDicts(data_args.data_path, train_repos, data_args))
        if len(eval_repos) != 0:
            eval_dataset = FlattenDatasetDict(GetCodeEditDatasetDicts(data_args.data_path, eval_repos, data_args))
            all_idx = [i for i in range(len(eval_dataset))]
            random.shuffle(all_idx)
            eval_dataset =  Subset(eval_dataset, all_idx[:int(len(eval_dataset)*data_args.validation_split_percentage/100)])
        else:
            eval_num = int(len(train_dataset)*data_args.validation_split_percentage*0.01)
            train_dataset, eval_dataset = torch.utils.data.random_split(train_dataset, [len(train_dataset) - eval_num, eval_num])
        train_dataloader = DataLoader(
            train_dataset, shuffle=True, collate_fn=data_collator, batch_size= training_args.per_device_train_batch_size
        )
        eval_dataloader = DataLoader(
            eval_dataset, shuffle=True, collate_fn=data_collator, batch_size= training_args.per_device_train_batch_size*2
        )
        ...
        train_dataloader, eval_dataloader, lr_scheduler = accelerator.prepare(train_dataloader, eval_dataloader, lr_scheduler)
        ...
        def RunEvaluation():
            ...
            for step, batch in enumerate(eval_dataloader):
                ...
                losses.append(accelerator.gather_for_metrics(loss.repeat(batch["input_ids"].shape[0])))
                # print(F"{k}:{step}\tbatch: {batch['input_ids'].shape}\tlosses: {losses[-1].shape}")
            # print(F"losses List: ({len(losses)}){losses}")
    ...
    # Train!
        ...
        progress_bar = tqdm(range(int(training_args.max_train_steps)), disable=not accelerator.is_local_main_process)
        completed_steps = 0
        starting_epoch = 0
        ...
        for epoch in range(starting_epoch, training_args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if completed_steps % training_args.logging_steps == 0 and completed_steps > 0 and step > 0:
                    accelerator.log(
                        {
            ...
            if training_args.output_dir is not None:
                output_dir = os.path.join(training_args.output_dir, output_dir)
    # print(f"Starting {accelerator.process_index}")
        if best_step != 0:
            accelerator.load_state(os.path.join(training_args.output_dir,F"step_{best_step}"))
    ...
    if training_args.do_eval:
        eval_datasets = GetCodeEditDatasetDicts(data_args.data_path, eval_repos, data_args) 
        eval_dataloaders = {k: DataLoader(
            v, collate_fn=data_collator, batch_size= get_batch_size(len(v), training_args.per_device_eval_batch_size, accelerator.num_processes)
        ) for k,v in eval_datasets.items()}
        TestAndDumpResults(accelerator,model,tokenizer, eval_dataloaders, data_args.max_seq_length, os.path.join(training_args.output_dir, "DevEvaluationBeamSearch"), 5, True)
        
    if training_args.do_test:
        logger.info(F"Evaluating with Beam Search")
        test_datasets = GetCodeEditDatasetDicts(data_args.data_path, test_repos, data_args) 
        test_dataloaders = {k: DataLoader(
            v, collate_fn=data_collator, batch_size= get_batch_size(len(v), training_args.per_device_eval_batch_size, accelerator.num_processes)
        ) for k,v in test_datasets.items()}
        
        TestAndDumpResults(accelerator,model,tokenizer, test_dataloaders, data_args.max_seq_length, os.path.join(training_args.output_dir, "TestEvaluationBeamSearch"), 5, True)
