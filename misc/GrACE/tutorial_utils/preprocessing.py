...
def get_diff_chunks(prev_file_lines, curr_file_lines, max_num_chunks=1):
    # get diff chunks
    diff_chunks = []
    diff = list(difflib.unified_diff(prev_file_lines, curr_file_lines, fromfile='v0', tofile='v1', lineterm=''))[2:]
    for line in diff:
        # print(line)
        if line.startswith('@@'):
...
def save_processed_example_json(processed_example_content, processed_example_json_file_path, id):
