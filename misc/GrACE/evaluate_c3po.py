...
from src.dataset import (
    AssociatedEditsDataset,
    AssociatedEditsFewShotDataset,
    AssociatedEditsFewShotAblationDataset,
)
...
def evaluate(args):
    ...
    for path in input_dir.iterdir():
        if path.suffix == ".json" and path.is_file() and path.stem in test_split:
            # Basic few-shot evaluation with k randomly sampled edits
            if use_fewshot_base:
            ...
            # Ablation with k associated edits and m randomly sampled edits (effect of noise)
                ...
                testDataset = AssociatedEditsDataset(
                    json_addr=path, use_ascEdits=use_ascEdits
                )
...
def parse_args():
    ...
    parser.add_argument(
        "--splits", type=str, help="JSON file containing the train/val/test splits"
    )
    parser.add_argument(
        "--output_dir", type=str, help="Output Directory to dump the precition data."
    )
    parser.add_argument(
        "--use_ascEdits", action="store_true", help="Use Associated Edits in the Prompt"
    )
    parser.add_argument(
        "--dumpFV",
        action="store_true",
        help="Also consider Future Versions of the code while evaluating (used for Overwatch evaluations)",
    )
    parser.add_argument(
        "--use_fewshot_base",
        action="store_true",
        help="Set to true to run basic few-shot experiments",
    )
    parser.add_argument(
        "--use_fewshot_ablation",
        action="store_true",
        help="Set to true to run few-shot 2+1 ablations",
    )
    parser.add_argument(
        "--few_shot_key",
        type=str,
        default="few_shot_samples_same_repo",
        help="Few Shot Key for choosing associated edits",
    )
    parser.add_argument(
        "--api_engine",
        type=str,
        choices=["oai", "azureoai", "hf"],
        default="openai",
        help="Either oai, azureoai or hf for OpenAI, Azure OpenAI or HuggingFace API Engine respectively. Use hf for CodeT5 models.",
    )
    parser.add_argument(
        "--model_name_or_path",
        type=str,
        default="text-davinci-003",
        help="Either name of the OpenAI/HuggingFace model or path to the model directory",
    )
    parser.add_argument(
        "--num_generations",
        type=int,
        default=5,
        help="Number of predictions to generate per sample",
    )
    parser.add_argument(
        "--max_tokens",
        type=int,
        default=256,
        help="Maximum number of tokens in the generation, longer sequences will be truncated",
    )
    parser.add_argument(
        "--temperature", type=float, default=0.1, help="Temperature used for sampling"
    )
