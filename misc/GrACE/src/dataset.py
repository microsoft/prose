...
class AssociatedEditsDataset(Dataset):
    def __init__(self, json_addr, assc_edit_prompt = lambda x : "<edit>\n{}</edit>".format("\n".join(["<{}>\n{}\n</{}>".format(k,x[k],k) for k in ["prefix", "before", "after", "suffix"]])), predict_prompt = lambda x : x["after"], curr_edit_prefix = lambda x: "<edit>\n" + "".join(["<{}>\n{}\n</{}>".format(k,x[k],k) for k in ["prefix","before"]]) + "\n<after>\n", curr_edit_suffix = lambda x: "\n</after>\n<suffix>\n{}\n</suffix>\n</edit>".format(x['suffix']), use_all = False, shuffle = False, use_ascEdits = True):
        self.dataLis = []
            ...
            for editSeqData in sessionData:
                ...
                for i, editData in enumerate(editSeqData["data"]):
                    ...
                    if use_ascEdits:
                        ...
                        ascEditCtx = "\n".join([assc_edit_prompt(ed) for ed in editData["AssociatedEdits"]])
                    ...
                    self.dataLis.append({"Spctx": prefix, "FutureCtx": suffix, "ExpectedText": predictText, "Id": id, "current_prefix": full_prefix, "current_suffix": full_suffix})
...
class AssociatedEditsFewShotDataset(Dataset):
    def __init__(self, json_addr, assc_edit_prompt = lambda x : "<edit>\n{}</edit>".format("\n".join(["<{}>\n{}\n</{}>".format(k,x[k],k) for k in ["prefix", "before", "after", "suffix"]])), predict_prompt = lambda x : x["after"], curr_edit_prefix = lambda x: "<edit>\n" + "".join(["<{}>\n{}\n</{}>".format(k,x[k],k) for k in ["prefix","before"]]) + "\n<after>\n", curr_edit_suffix = lambda x: "\n</after>\n<suffix>\n{}\n</suffix>\n</edit>".format(x['suffix']), use_all = False, shuffle = False, use_asc_edits = True, few_shot_key="few_shot_samples_same_repo"):
        self.dataLis = []
        filePath = Path(json_addr)
        self.mode = "lineComp"
        if filePath.suffix == ".json" :
            ...
            for editSeqData in sessionData:
                ...
                for i, editData in enumerate(editSeqData["data"]):
                    ...
                    if use_asc_edits:
                        ...
                        asc_edits = [fewShotSample["Assoc_Edit"] for fewShotSample in editData[few_shot_key]]
                        asc_edit_ids = [fewShotSample["sample_id"] for fewShotSample in editData[few_shot_key]]
