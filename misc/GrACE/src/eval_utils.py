...
def dumpid2pred(id2pred, target_file, dumpFV=False):
    ...
    with open(target_file, "w+", newline="", encoding="utf-8") as csvfile:
        ...
        if (
            len(id2pred) > 0
            and "k-shotAscEditIds" in id2pred[list(id2pred.keys())[0]]["pred"]
        ):
        ...
        for id in id2pred:
            rowDict = {
                ...
                "RawPredTop-1": [
                    rawPred[0] if len(rawPred) > 0 else " "
                    for rawPred in id2pred[id]["pred"]["topPreds"]
                ],
                "RawPredTop-2": [
                    rawPred[1] if len(rawPred) > 1 else " "
                    for rawPred in id2pred[id]["pred"]["topPreds"]
                ],
                "RawPredTop-3": [
                    rawPred[2] if len(rawPred) > 2 else " "
                    for rawPred in id2pred[id]["pred"]["topPreds"]
                ],
                "RawPredTop-4": [
                    rawPred[3] if len(rawPred) > 3 else " "
                    for rawPred in id2pred[id]["pred"]["topPreds"]
                ],
                "RawPredTop-5": [
                    rawPred[4] if len(rawPred) > 4 else " "
                    for rawPred in id2pred[id]["pred"]["topPreds"]
                ],
