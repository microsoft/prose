...
def truncateLineListsByLineCount(
    currentLines: list[str],
    prefixLines: list[str],
    suffixLines: list[str],
    lineCount=10,
):
    ...
    if tokensLine > lineCount:
        ...
        truncatedPrefixLines, nonTrivialLines = TryGetFirstKNonTrivialLines(
            prefixLines.__reversed__(), lineCount // 2
        )
        truncatedPrefixLines.reverse()
        truncatedSuffixLines, _ = TryGetFirstKNonTrivialLines(
            suffixLines, lineCount - nonTrivialLines
        )
    return {
...
def truncateLineListsByLineCountAndGetDiffDict(
    prefix, suffix, before, after, lineCount
):
    before = truncateLineListsByLineCount(
        lineCount=lineCount, prefixLines=prefix, suffixLines=suffix, currentLines=before
    )
    after = truncateLineListsByLineCount(
        lineCount=lineCount, prefixLines=prefix, suffixLines=suffix, currentLines=after
    )
    suffix = (
        before["suffix"]
        if len(before["suffix"]) < len(after["suffix"])
        else after["suffix"]
    )
    prefix = (
        before["prefix"]
        if len(before["prefix"]) < len(after["prefix"])
        else after["prefix"]
    )
    return {
        "prefix": "".join(prefix),
...
def LocalizeChanges(beforeLines: list[str], afterLines: list[str]):
    ...
    prefix = match["prefix"]
    match = match_prefix(
        list(reversed(match["before"])), list(reversed(match["after"]))
    )
...
def ASTLocalizationToLineLocalization(edDict: dict[str, list[str]]):
    ...
    if ((b := TryGetLast(retEdDict["before"])) and not isCompleteLine.match(b)) or (
        (a := TryGetLast(retEdDict["after"])) and not isCompleteLine.match(a)
    ):
...
def processEditSeq(editSeq):
    ...
    for k in range(len(allEdits)):
        ...
        localizedEdits = [
            LocalizeChanges(
                "".join(allAfter[:i] + allBefore[i:]).splitlines(keepends=True),
                "".join(allAfter[: i + 1] + allBefore[i + 1 :]).splitlines(
                    keepends=True
                ),
            )
            for i in range(k)
        ...
        localizedEdits.extend(
            [
                ...
                for i in range(k + 1, len(allBefore))
            ]
        )
        truncEdits = [
            truncateLineListsByLineCountAndGetDiffDict(
                lineCount=(8 if i != k else 15), **ed
            )
            for i, ed in enumerate(localizedEdits)
        ]
        dataLis.append(
            {
    ...
    return {"id": editSeq["id"], "data": dataLis}
