...
def fetch_dataset(dir_path):
    ...
    repo_json_dict = {
        file_name.split(".")[0]: os.path.join(dir_path, file_name)
...
def tag_unfiltered_data_with_filtered_ids(filtered_data, unfiltered_data):
    ...
    for repo in repos:
        ...
        for sample in tqdm.tqdm(
            filtered_data[repo], desc="Matching samples, adding filtered ids"
        ):
            # Find all samples in unfiltered data that match the current sample
            match = [
                unfiltered_sample
                for unfiltered_sample in unfiltered_data[repo]
                if unfiltered_sample["data"][1]["Current"]
                == sample["data"][1]["Current"]
            ]
            for m in match:
...
def get_random_assoc_edits(data, id_key, target_id, num_samples, seed=42):
    ...
    if len(candidate_samples) < num_samples:
        print(
            f"Warning: Not enough candidate samples to pick {num_samples} samples for {target_id}. Picking {len(candidate_samples)} samples instead."
        )
        return [
            {
                "sample_id": sample["id"],
                "Assoc_Edit": sample["data"][1]["Current"],
...
def create_and_dump_few_shot_dataset(
    target_samples_by_repo,
    few_shot_candidate_samples_by_repo,
    num_few_shot_samples,
    candidate_id_key,
    target_id_key="id",
    output_dir=None,
    seed=42,
    ...
    for repo in target_repos:
        ...
        for target_sample in tqdm.tqdm(
            target_samples_by_repo[repo], desc="Creating few-shot dataset"
        ):
            target_id = target_sample[target_id_key]
            target_sample["data"][1][
                "few_shot_samples_same_repo"
            ] = get_random_assoc_edits(
                few_shot_candidate_samples_by_repo[repo],
                candidate_id_key,
                target_id,
                num_few_shot_samples,
            )
            target_sample["data"][1][
                "few_shot_samples_any_repo"
            ] = get_random_assoc_edits(
                all_few_shot_candidate_samples,
                candidate_id_key,
                target_id,
        ...
        if output_dir is not None:
            ...
            print(
                f"Dumped few-shot dataset for {repo} in {output_dir} with {len(target_samples_by_repo[repo])} samples."
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--filtered_input_dir",
        type=str,
        help="Directory containing the filtered C3PO dataset.",
    )
    parser.add_argument(
        "--unfiltered_input_dir",
        type=str,
        help="Directory containing the unfiltered C3PO dataset.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Number of few-shot samples to collect for each sample.",
    )
    ...
    create_and_dump_few_shot_dataset(
        filtered_data,
        filtered_data,
        num_few_shot_samples=num_few_shot_samples,
        candidate_id_key="id",
        output_dir=filtered_output_root,
        seed=seed,
    )
    ...
    create_and_dump_few_shot_dataset(
        ...
        unfiltered_data,
        num_few_shot_samples=num_few_shot_samples,
        candidate_id_key="filtered_id",
        output_dir=unfiltered_output_root,
        seed=seed,
    )
